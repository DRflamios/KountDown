<template>
    <main class="flex h-screen overflow-hidden" :class="{ 'dark': isDark }">
        <div class="w-1/3 p-6 bg-gray-50 border-r dark:bg-gray-800 dark:border-gray-700">
            <div class="flex flex-col gap-8">
                ...
                <section class="p-4 bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-center justify-center bg-gray-100 dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-600">
                        <input type="number" v-model="manualTimer.minutes" name="minutes" id="minutes" min="0" class="p-2 border w-20 rounded remove-arrow text-center text-3xl dark:bg-gray-900 dark:text-white dark:border-gray-600">
                        <span class="mx-2 text-3xl dark:text-white">:</span>
                        <input type="number" v-model="manualTimer.seconds" name="seconds" id="seconds" min="0" class="p-2 border w-20 rounded remove-arrow text-center text-3xl dark:bg-gray-900 dark:text-white dark:border-gray-600">
                    </div>
                    <div class="mt-4">
                        <label for="manualSpeakerLang" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ $t('dashboard.speakerlang') }}</label>
                        <select v-model="manualTimer.speakerLang" id="manualSpeakerLang" class="w-full p-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md text-sm dark:text-white">
                            <option value="en">{{ $t('dashboard.langs.en') }}</option>
                            <option value="fr">{{ $t('dashboard.langs.fr') }}</option>
                            <option value="no">{{ $t('dashboard.langs.no') }}</option>
                        </select>
                    </div>
                    <button class="mt-4 px-6 py-3 w-full rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-semibold text-lg shadow-sm transition" @click="startManualTimer" :disabled="isQueueRunning">{{ $t('dashboard.manualTimer.start') }}</button>
                </section>
                ...
                <section class="p-4 bg-white rounded-lg shadow dark:bg-gray-700 flex flex-col h-full">
                    ...
                    <form @submit.prevent="addToQueue" class="flex flex-col gap-4 mb-4">
                        <input v-model="newTimer.speaker" type="text" :placeholder="$t('dashboard.queue.speaker')" class="p-2 border rounded dark:bg-gray-800 dark:text-white dark:border-gray-600" required />
                        <div class="flex items-center gap-2">
                            <input v-model.number="newTimer.minutes" type="number" :placeholder="$t('dashboard.queue.minutes')" class="p-2 border w-20 rounded remove-arrow text-center dark:bg-gray-800 dark:text-white dark:border-gray-600" required />
                            <span class="text-xl dark:text-white">:</span>
                            <input v-model.number="newTimer.seconds" type="number" :placeholder="$t('dashboard.queue.seconds')" class="p-2 border w-20 rounded remove-arrow text-center dark:bg-gray-800 dark:text-white dark:border-gray-600" required />
                        </div>
                        <div>
                            <label for="queueSpeakerLang" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ $t('dashboard.speakerlang') }}</label>
                            <select v-model="newTimer.speakerLang" id="queueSpeakerLang" class="w-full p-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md text-sm dark:text-white">
                                <option value="en">{{ $t('dashboard.langs.en') }}</option>
                                <option value="fr">{{ $t('dashboard.langs.fr') }}</option>
                                <option value="no">{{ $t('dashboard.langs.no') }}</option>
                            </select>
                        </div>
                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded self-end" :disabled="isManualRunning">{{ $t('dashboard.queue.add') }}</button>
                    </form>

                    <div class="flex-1 overflow-y-auto border rounded divide-y dark:divide-gray-700">
                        <ul>
                            <li v-for="(timer, index) in queue" :key="index" class="flex justify-between items-center p-2 dark:text-white" :class="{ 'bg-green-100 dark:bg-green-800': activeQueueIndex === index }">
                                <div>
                                    <strong>{{ timer.speaker }}</strong>: {{ timer.minutes }}m {{ timer.seconds }}s
                                </div>
                                <div class="flex gap-2">
                                    <button @click="removeFromQueue(index)" class="px-2 py-1 bg-red-500 text-white rounded">{{ $t('dashboard.queue.remove') }}</button>
                                </div>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="mt-4">
                        <button class="px-6 py-3 w-full rounded-lg bg-green-500 hover:bg-green-600 text-white font-semibold text-lg shadow-sm transition" @click="startNextTimer" :disabled="isManualRunning || isQueueRunning">{{ $t('dashboard.queue.startNext') }}</button>
                    </div>
                </section>
            </div>
        </div>
        ...
        <div class="w-2/3 bg-white dark:bg-gray-900 relative">
            ...
            <div v-if="isManualRunning || isQueueRunning" class="absolute bottom-4 right-4 p-4 bg-white rounded-lg shadow-lg dark:bg-gray-800">
                <h2 class="text-lg font-semibold mb-2 dark:text-white">{{ $t('dashboard.generalControls.title') }}</h2>
                <div class="flex gap-4">
                    <button class="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white font-semibold shadow-sm transition" @click="stopTimer">{{ $t('dashboard.generalControls.stop') }}</button>
                    <button v-if="!isPaused" class="px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-white font-semibold shadow-sm transition" @click="pauseTimer">{{ $t('dashboard.generalControls.pause') }}</button>
                    <button v-else class="px-4 py-2 rounded-lg bg-indigo-500 hover:bg-indigo-600 text-white font-semibold shadow-sm transition" @click="resumeTimer">{{ $t('dashboard.generalControls.resume') }}</button>
                </div>
            </div>
        </div>
    </main>
